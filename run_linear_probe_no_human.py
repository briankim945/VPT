...
def train_linear_probe(model, train_loader, test_loader, val_loader, criterion, optimizer, lr_scheduler, device, args):
    best_acc_val = 0
    best_acc_test = 0
    best_acc_train = 0
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if args.wandb:
            wandb.log({'train_acc':train_acc, 'train_loss':sum(epoch_loss)/float(len(epoch_loss)),
                    'val_acc':val_acc, 'val_loss':val_loss, 'test_acc':test_acc,
                    'test_loss':test_loss})
        if args.early_stop and early_stopper.early_stop(val_loss):             
            break
    return best_acc_train, best_acc_test, best_acc_val
...
def run_linear_probe(args):
    ...
    val_features, val_labels = run_extract_features(args)
    ...
    best_acc_train, best_acc_test, best_acc_val = train_linear_probe(linear_model, train_feat_loader, 
                                                            test_feat_loader, val_feat_loader, 
    ...
    with open(log_file, 'r') as f:
        results = json.load(f)
    results[args.model_name] = [best_acc_train, best_acc_test, best_acc_val]
